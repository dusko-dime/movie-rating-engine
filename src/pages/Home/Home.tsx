...
const Home = () => {
    ...
    const fetchMovieSearchResults: () => void = async () => {
        try {
            ...
            const data = await fetchMovies({
                searchText: searchText && searchText.length > 1 ? searchText : undefined,
                searchOffset,
    ...
    const loadMoreMovieSearchResults: () => void = async () => {
        try {
            ...
            if(requestId === requestIdCounter.current) {
                ...
                if (data) {
                    if (movies)
                        newMovies = [...movies, ...(data as Movie[])];
                    else
                        newMovies = data as Movie[];
    ...
    const onChangeSearchValue = (e: any) => {
        if((searchText.length === 1 && e.target.value.length === 0) || (searchText.length === 0 && e.target.value.length === 1)) {
    ...
    return (
        <HomeContainer>
            ...
            <FiltersBox>
                <SearchBox>
                    <SearchInput onChange={onChangeSearchValue} value={searchText} placeholder='Search movie...' />
                </SearchBox>
                <TopMoviesSwitchBox>
                    <TopMoviesLabel>
                        See TV Shows only?
                    </TopMoviesLabel>
                    <TopMoviesSwitch checked={isShowFilterActive} onChange={onShowSwitchChangeValue}/>
                </TopMoviesSwitchBox>
            </FiltersBox>
            <MoviesBox>
                {isLoading ? <AppLoader/> : <><MovieList movies={movies}/>
                    <LoadMoreButtonBox>
                        {moreItemsExist && <LoadMoreButton onClick={onLoadMoreClick}>
                            Load more
                        </LoadMoreButton>}
                    </LoadMoreButtonBox>
                </>
