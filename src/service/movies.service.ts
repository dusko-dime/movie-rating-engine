...
export const fetchMovies = async ({searchText, searchOffset, tvShowsMode}: FetchMoviesReqParam) => {
    return new Promise((resolve, reject) => [
        setTimeout(() => {
            let filteredMovies = movieDB;

            if (tvShowsMode)
                filteredMovies = filteredMovies.filter(movie => movie.type === Type.SHOW);
            else
                filteredMovies = filteredMovies.filter(movie => movie.type === Type.MOVIE);
            ...
            if (searchText) {
                ...
                KnownPhrases.forEach((knownPhrase) => {
                    ...
                    let doesMatchTest = searchText.toLowerCase().match(knownPhrase.regexTest);
                    ...
                    if (doesMatchTest && !knownPhraseFound) {
                        ...
                        switch (knownPhrase.key) {
                            case PhraseKeys.AtLeastNumberOfStars:
                                // @ts-ignore
                                searchNumber = parseFloat(searchText.match(/[+-]?([0-9]*[.])?[0-9]+/g)[0]);
                                filteredMovies = filteredMovies.filter(movie => movie.averageRating >= searchNumber)
                                break;
                            ...
                            case PhraseKeys.OlderThan:
                                ...
                                filteredMovies = filteredMovies.filter(movie => {
                                    const movieReleaseDatePlusOffset = new Date(movie.releaseDate.getFullYear() + searchNumber, movie.releaseDate.getMonth(), movie.releaseDate.getDay());
                                    return movieReleaseDatePlusOffset.getTime() < currentTime.getTime();
                                })
                                break;
                            default:
            ...
            if (searchOffset) {
                filteredMovies = filteredMovies.slice(searchOffset, searchOffset + Settings.moviesLoadingStep)
            } else
...
export const postRating = async ({movieId, numberOfStars}: PostRatingReqParam) => {
...
// ----------------------------------------------------------------------------------------------------
export const getMoviesApi = async ({searchText, searchOffset, tvShowsMode}: FetchMoviesReqParam) => {
...
export const postRatingApi = async ({movieId, numberOfStars}: PostRatingReqParam) => {
